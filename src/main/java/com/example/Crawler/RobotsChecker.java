...
{
    private static final ConcurrentMap<String, RobotsTxtCache> domainCache =
            new ConcurrentHashMap<>();
    public static final String USER_AGENT = "MyWebCrawler/1.0";
    private static final long DEFAULT_DELAY = 500; // 0.5 second instead of 1 second
    private static final int MAX_CACHE_SIZE = 1000; // Limit the cache size to prevent memory issues
    ...
    private static RobotsTxtCache getRobotsTxtCache(String domain) throws IOException
    {
        ...
        if (domainCache.size() > MAX_CACHE_SIZE)
        {
            ...
            List<String> keys = new ArrayList<>(domainCache.keySet());
            Collections.shuffle(keys);
            for (int i = 0; i < keys.size() * 0.2; i++)
            {
                domainCache.remove(keys.get(i));
            }
        }
    ...
    private static String fetchRobotsTxt(String domain) throws IOException
    {
        ...
        for (int i = 0; i < maxRetries; i++)
        {
            try
            {
                return Jsoup.connect(robotsUrl).userAgent(USER_AGENT).ignoreContentType(true)
            ...
            catch (IOException e)
            {
                if (i == maxRetries - 1)
                    throw e;
                System.out.println("Retrying robots.txt fetch for " + robotsUrl + " (attempt "
                        + (i + 1) + ")");
                try
                {
